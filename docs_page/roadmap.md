...
- **Risk — critical:** The bounded codec covers observed containers, records, field descriptions, exact literal/compressed elementary INT4/CHAR/STRING `0x5001` items, and LZ4 block production, but composite/table parameter graphs, responses, exceptions, WAN/zlib compression and mode selection, version negotiation, and integration are not yet complete. Guessing at those gaps could cause protocol corruption and an unmaintainable stable claim. Domains: `correctness`, `security`, `scope`, `external-dependency`, `schedule`.
